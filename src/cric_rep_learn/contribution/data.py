"""Build leakage-safe batting-stint datasets for expected contribution."""
    ...
    connection.execute(
        ...
        aggregated AS (
            SELECT
                ...
                MIN(attempt_index_in_innings)::INTEGER AS first_attempt_index
            FROM ranked
            GROUP BY 1, 2, 3, 4
        ),
        entry AS (
        ...
        SELECT
            ...
            s.split,
            CASE
                WHEN e.innings = 1 THEN 'first_innings'
                ELSE 'chase'
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("artifacts/contribution-data"))
